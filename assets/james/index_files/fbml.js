/*       Source:  Local Cache                                                                 */
/*     Location:  rsrc@81610@/html/js/fbml.js                                                 */
/*      Machine:  10.1.0.46                                                                   */
...
function attachFromPreview(){if(attachments){var parent=ge('attachment_edit');var inputs=attachments.get_all_form_elements(parent);var params=Object();for(var i=0;i<inputs.length;i++){if(!(inputs[i].type=="radio"||inputs[i].type=="checkbox")||inputs[i].checked){params[inputs[i].name]=inputs[i].value;}}
...
function cancelInteraction(elem){generic_dialog.get_dialog(elem).fade_out(100);}
function sendInteraction(interaction_form,app_id,preview,prefill){var ids=[];if(prefill){ids.push(prefill);}else{var inputs=interaction_form.getElementsByTagName('input');for(var i=0;i<inputs.length;i++){if(inputs[i].getAttribute('fb_protected')=='true'&&(inputs[i].name=='ids[]'||inputs[i].name=='friend_selector_id')&&(inputs[i].type!='checkbox'||inputs[i].checked)){ids.push(inputs[i].value);}}}
var data={app_id:app_id,to_ids:ids,template:interaction_form.getAttribute('template'),preview:preview,form_id:interaction_form.id,prefill:(prefill>0),elements:serialize_form(interaction_form)};var ajax_uri='/fbml/ajax/prompt_interaction.php';if(preview){var dialog=new pop_dialog();dialog.show_ajax_dialog(ajax_uri,data);}else{new AsyncRequest().setURI(ajax_uri).setData(data).setHandler(function(){interaction_form.submit();}).setErrorHandler(function(err){aiert('error: '+err);}).send();}
return false;}
return{friendSelector:friendSelector,Contexts:Contexts,attachCurlFromObject:attachCurlFromObject,attachFromPreview:attachFromPreview,clickRewriteAjax:clickRewriteAjax,clickToShow:clickToShow,clickToShowDialog:clickToShowDialog,clickToHide:clickToHide,clickToEnable:clickToEnable,clickToDisable:clickToDisable,clickToToggle:clickToToggle,closeDialogInvoked:closeDialogInvoked,cancelRequest:cancelRequest,sendRequest:sendRequest,removeReqRecipient:removeReqRecipient,cancelInteraction:cancelInteraction,sendInteraction:sendInteraction,addHiddenInputs:addHiddenInputs,fbmlLogin:fbmlLogin,cancelLogin:cancelLogin,requireLogin:requireLogin};})();function fbjs_sandbox(appid){if(fbjs_sandbox.instances['a'+appid]){return fbjs_sandbox.instances['a'+appid];}
...
return;}else{var xml=fbjs_ajax.get_transport(this,true);if(xml&&post_form_id){this.status=fbjs_ajax.STATUS_WAITING_FOR_SERVER;xml.open('POST',fbjs_ajax.proxy_url,true);xml.setRequestHeader('Content-Type','application/x-www-form-urlencoded');xml.send(URI.implodeQuery({url:url,query:query,post_form_id:post_form_id.value,type:this.responseType,require_login:this.requireLogin,fb_mockajax_context:fbjs_sandbox.instances['a'+appid].contextd,fb_mockajax_context_hash:fbjs_sandbox.instances['a'+appid].context,appid:appid}));fbjs_private.get(this).inflight=true;}else if(this.onerror){this.onerror();}else{fbjs_console.error('There was an uncaught Ajax error. Please attach on onerror handler to properly handle failures.');}}}
...
function fbjs_dialog(appid){var proto=function(type){var priv=fbjs_private.get(this);switch(type){case fbjs_dialog.DIALOG_CONTEXTUAL:priv.dialog=new contextual_dialog('app_content_'+appid);break;case fbjs_dialog.DIALOG_POP:default:priv.dialog=new pop_dialog('app_content_'+appid);break;}
...
return domElement;};
if(typeof(ScriptInventory)!='undefined'){ScriptInventory.addScript("\/samba\/www-patchtable\/\/html\/js\/fbml.js", "81610", 1202544027);}