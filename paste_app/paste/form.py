...
def delete_token(ip_address, stoken):
    token = paste_app.paste.model.Form()
    token.token = stoken
...
def has_valid_token(ip_address, token):
    tokens = paste_app.paste.model.Form.all()
...
def put_form_token(ip_address, token=""):
    ...
    ftoken = paste_app.paste.model.Form()
    ftoken.token = token
    ftoken.created_at = datetime.datetime.now()
    ftoken.created_by_ip = ip_address
    ftoken.expired_at = datetime.datetime.now() + paste_app.paste.config["pasty_form_expiration_delta"]
    key = ftoken.put()
    if key != "":
        return token
    else:
